"""
Early-comment enrichment for the Hacker News domain: the first comments a
story received, as a point-in-time-safe leading indicator of reception.

WHY THIS EXISTS. Submission-only seeds hit a measured ceiling: author karma
alone predicts the points direction at 69%, no other static field beats it,
and the simulation doesn't clear it (METHODOLOGY.md). Early community
reaction is the one signal class genuinely different in kind, dynamic
rather than static, and so the one lever with real headroom.
...
- Only comments created within ``window_seconds`` of the story's own creation
  are returned. A comment at T+30min is observable at T+30min; one at T+10h
  isn't, and would leak the future into the seed.
- ``window_seconds`` must stay well below the 24h settlement window, or the
  "early" observation would overlap the outcome it's meant to predict.
...
- Points and num_comments are still never read or stored. The comment count
  ...
  target (a strict prefix of it), legitimately observable at T+window.
...
strictly easier than the submission-only task, so accuracy numbers aren't
comparable between them. Any baseline must get the same window (see
``early_engagement_prediction``) or the comparison favors the simulation.

CAVEAT ON THE COMMENTS AXIS. Early comment count is a strict prefix of the
24h num_comments target, so on that axis it partially self-predicts. Points
is the honest axis here; treat comments-axis numbers as compromised.
"""
...
# Two hours. Long enough that a story getting traction usually shows some,
# short enough to leave 22 of 24 settlement hours unobserved. A first-pass
# constant, not calibrated against backtest accuracy.
DEFAULT_EARLY_WINDOW_SECONDS = 2 * 60 * 60

# The early window must be a small fraction of the settlement window, not
# merely less than it: observing 23 of 24 hours would be "early" by the
# letter of the rule while being almost the whole outcome.
_MAX_WINDOW_FRACTION = 0.25

_MAX_EARLY_COMMENTS = 12
_MAX_COMMENT_CHARS = 280

# 2+ comments inside the early window as the traction cutoff. Chosen a
# priori from HN's shape (most submissions get zero discussion), not tuned
# against the backtest, a fitted threshold would make this baseline
# unfairly strong and the comparison meaningless.
